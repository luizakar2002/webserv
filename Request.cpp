...
Request::Request(std::string request): status_code(200), body("")
{
    this->request = request;
    this->parse_first_line(this->request);
    this->parse_headers(this->request);
    this->parse_body(this->request);
...
void    Request::parse_first_line(std::string request)
{
    std::string protocol;
...
void    Request::parse_headers(std::string request)
{
    std::string header;
    std::string key;
    std::map<std::string, std::string>::iterator it;

    std::string headers = request.erase(0, request.substr(0, request.find("\r\n")).size() + 2);
...
{
    this->body = request.substr(request.find("\r\n\r\n") + 4);
